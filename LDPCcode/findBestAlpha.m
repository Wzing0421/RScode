...
for a = 0:0.1:1
    ...
    while 1
        ...
        u = zeros(1008,2016);
        v = zeros(2016,1008);
        [isSuc, errorframenum, errorbitnum] = Decode_MinSum( LDPCRecv, H_index, H_index_len, H_var, H_var_len, u, v, H_ldpc, LDPCEnCode, a );
        ...
        %循环退出的判决条件
        if(totalerror_framenum > 100) %这是跳出循环的条件：误帧数超过100 
            break;
        end
    ...
    error_bitrate(iter,round(10 * a + 1)) = totalerror_bitnum/(lp*1008);%只需要计算误码率
...
title ('LDPC different α code Performace');
xlabel('α的值'); 
ylabel('BER');
